...
- **Phase 2 (crypto + extrinsic):** SR25519/ED25519 signing, Blake2b hashing,
  ...
  audited constants.
- **Phase 3 (crypto oracle):** SR25519-verify and Blake2b-256 known-answer
  vectors generated from the Kotlin oracle (`UosCryptoCorpusExport`).  Rust
  verifies SR25519 signatures; it never re-signs (schnorrkel uses OS RNG).
...
**UOS protocol corpus** (payload, signature, multipart, account-introduction):
...
**Crypto oracle corpus** (SR25519-verify and Blake2b-256):
...
├── crypto/               # Crypto KAT vectors  (Phase 2 + Phase 3 Kotlin oracle)
│   ├── ss58_encode.json        # Alice pubkey + prefix 42 → SS58 address
│   ├── ss58_decode.json        # Alice SS58 address → pubkey + prefix
│   ├── blake2b_256.json        # Blake2b-256 hand-picked reference vectors  (Phase 2)
│   ├── blake2b_256_oracle.json # Blake2b-256 Kotlin oracle vectors  (Phase 3)
│   └── sr25519_verify.json     # SR25519 verify Kotlin oracle vectors  (Phase 3)
...
### Phase 3 — crypto/sr25519_verify.json

Generated by `UosCryptoCorpusExport.exportSr25519VerifyCorpus` using
`sr25519-kotlin` (pure Kotlin schnorrkel).  Kotlin signs each vector; Rust only
*verifies*.  Signatures are not byte-stable across runs (OS RNG); the committed
file freezes one valid signature per vector for Rust to verify indefinitely.
  ...
  "vectors": [
    {
      ...
      "expected_signature_hex": "<128 hex chars — 64 bytes>"
    }
  ]
}
```

Signing parameters: context `b"substrate"`, `ExpansionMode::Ed25519`.

### Phase 3 — crypto/blake2b_256_oracle.json

Generated by `UosCryptoCorpusExport.exportBlake2b256OracleCorpus` using
BouncyCastle `Blake2bDigest(256)` with no key, salt, or personalization —
matching Rust's `blake2::Blake2b::<U32>::new()`.  Supplements the Phase 2
hand-picked vectors in `blake2b_256.json`.
